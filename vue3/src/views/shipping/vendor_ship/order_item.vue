<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="itemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="店铺" align="left" prop="shopId" width="120">
...
function handlePushItem(){
  ...
  listAllSupplier({}).then((res:any)=>{
    const list=res.rows||[]
    if(list.length===0){ElMessage.warning('没有可用的供应商');return}
    ElMessageBox.prompt('请输入供应商ID','推送发货',{inputPlaceholder:'供应商ID',inputPattern:/^\d+$/,inputErrorMessage:'请输入数字ID'}).then(({value})=>{
      pushOrderItemToSupplier({ids,supplierId:value}).then((res:any)=>{if(res.code===200){ElMessage.success('推送成功');getList()}else ElMessage.error(res.msg||'推送失败')})
    }).catch(()=>{})
  })
}
onMounted(()=>{
